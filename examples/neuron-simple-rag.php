...
$requiredEnv = static function (string $name): string {
    ...
    if ($value === false || trim($value) === '') {
        throw new RuntimeException('Defina a variável obrigatória ' . $name . '.');
    }
    return trim($value);
};
...
try {
    ...
    if ($reranker instanceof PostProcessorInterface) {
        $postProcessors[] = $reranker;
    }
    $rag->setPostProcessors($postProcessors);
    $rag->addDocuments($documents);

    $message = new UserMessage($question);
    $sources = $rag->resolveRetrieval()->retrieve($message);
    foreach ($postProcessors as $processor) {
        $sources = $processor->process($message, $sources);
        if ($sources === []) {
            break;
        }
    }
